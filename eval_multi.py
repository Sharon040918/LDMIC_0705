    ...
    if data_name == 'cityscapes':
        ...
        if (path / "images" / "C1").is_dir():
            ...
            for idx in range(1, dir_num + 1):
                ...
                files.sort()
                train_num = int(train_test_ratio * len(files))
                if (train_num - num_camera + 1) % 2 == 1:
                    train_num += 1
                test_files = files[train_num:]
                for ii in range(len(test_files) - num_camera + 1):
                    for jj in range(num_camera):
                        image_lists[jj].append(test_files[ii + jj])
        return image_lists
...
def compute_metrics_for_frame(
    ...
    if org_frame.shape != rec_frame.shape:
        h = min(org_frame.size(2), rec_frame.size(2))
        w = min(org_frame.size(3), rec_frame.size(3))
        org_frame = org_frame[:, :, :h, :w]
        rec_frame = rec_frame[:, :, :h, :w]
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--num-camera", type=int, default=3, help="The number of cameras"
    )
    return parser
